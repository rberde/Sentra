...
test("authorizeStateRequest requires a configured matching server sync token", async () => {
  ...
  try {
    ...
    await withEnv({ NEXT_PUBLIC_PFRE_SYNC_TOKEN: "public-only" }, async () => {
      const result = authorizeStateRequest(new Request("https://sentra.test/api/state/sync", {
        headers: { "x-pfre-sync-token": "public-only" },
      }));
      assert.equal(result.ok, false);
      assert.equal(result.status, 503);
    });
...
test("deleteServerState only removes the matching token snapshot", async () => {
  const tmp = await mkdtemp(path.join(os.tmpdir(), "pfre-state-test-"));
  try {
    const { deleteServerState, readServerState, writeServerState } = await loadServerStateForCwd(tmp);

    await writeServerState("secret-a", { profile: { name: "A" } });
    await writeServerState("secret-b", { profile: { name: "B" } });

    await deleteServerState("secret-a");

    assert.equal(await readServerState("secret-a"), null);
    assert.deepEqual(await readServerState("secret-b"), { profile: { name: "B" } });
  } finally {
    await rm(tmp, { recursive: true, force: true });
  }
});
