...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "SET_REBALANCING_PLANS": {
      const existingSelection = state.rebalancingPlans.find(p => p.id === state.selectedPlanId);
      const selectedPlanId = action.plans.some(p => p.id === state.selectedPlanId)
        ? state.selectedPlanId
        : existingSelection
          ? action.plans.find(p => p.type === existingSelection.type)?.id ?? null
          : null;
