...
export async function GET(req: Request) {
  ...
  if (shouldRun("spending") && activePlan && reallocation) {
    ...
    const usage = calculateSpendingUsage(totalVariable, reallocation, threshold);
    const fired = usage.alert;
    alerts.push({
      check: "spending",
      alert: fired,
      severity: fired ? (usage.percentUsed >= 120 ? "critical" : "warning") : "info",
      title: "Spending Monitor",
      message: fired
        ? `Variable spending at ${usage.percentUsed}% of plan cap ($${totalVariable} / $${usage.variableCap}).`
        : `Spending within budget at ${usage.percentUsed}%.`,
      data: { variableCap: usage.variableCap, actualSpending: totalVariable, percentUsed: usage.percentUsed, threshold },
    });
  }
