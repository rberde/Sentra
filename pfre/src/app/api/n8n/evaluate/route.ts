...
import { calculateDriftCategories, calculateSpendingMonitor } from "@/lib/monitoring";
...
export async function GET(req: Request) {
  ...
  if (shouldRun("spending") && activePlan && reallocation) {
    ...
    const result = calculateSpendingMonitor(totalVariable, reallocation.variableExpenses ?? 0, threshold);
    alerts.push({
      check: "spending",
      alert: result.alert,
      severity: result.alert ? (result.alertLevel ?? "warning") : "info",
      ...
      message: result.alert
        ...
        : `Spending within budget at ${result.percentUsed}%.`,
      data: {
        variableCap: result.variableCap,
  ...
  if (shouldRun("drift") && activePlan && reallocation) {
    const drifts = calculateDriftCategories(income, [
