...
export async function GET(req: Request) {
  ...
  if (shouldRun("spending") && activePlan && reallocation) {
    ...
    const threshold = normalizeSpendingThreshold(spendingRule?.threshold as number | undefined);
  ...
  if (shouldRun("drift") && activePlan && reallocation) {
    const drifts = [
      { name: "Fixed", actual: totalFixed, planned: Math.round(reallocation.fixedExpenses ?? 0) },
      { name: "Variable", actual: totalVariable, planned: Math.round(reallocation.variableExpenses ?? 0) },
      { name: "Investments", actual: investments?.monthlyContribution ?? 0, planned: Math.round(reallocation.investments ?? 0) },
    ].map(d => ({
      ...d,
      drift: d.planned > 0 ? Math.round(Math.abs(d.actual - d.planned) / d.planned * 100) : 0,
    }));
