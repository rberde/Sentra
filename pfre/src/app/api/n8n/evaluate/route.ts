...
import { calculateDriftCategories, calculateSpendingUsage } from "@/lib/engine/monitoring";
...
export async function GET(req: Request) {
  ...
  if (shouldRun("spending") && activePlan && reallocation) {
    const spendingRule = rules.find(r => r.type === "spending_cap" && r.enabled) as
      | { threshold?: number; aiGenerated?: boolean }
      | undefined;
    const { variableCap, percentUsed, threshold, alert: fired } = calculateSpendingUsage(
      totalVariable,
  ...
  if (shouldRun("drift") && activePlan && reallocation) {
    const drifts = calculateDriftCategories(
      {
