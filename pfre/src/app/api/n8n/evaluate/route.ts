...
import { evaluateAllocationDrift, evaluateSpending } from "@/lib/engine/monitoring";
...
export async function GET(req: Request) {
  ...
  if (shouldRun("spending") && activePlan && reallocation) {
    ...
    const spending = evaluateSpending(
      totalVariable,
      reallocation.variableExpenses ?? 0,
      spendingRule,
    );
    alerts.push({
      check: "spending",
  ...
  if (shouldRun("drift") && activePlan && reallocation) {
    ...
    alerts.push({
      check: "drift",
      alert: drift.alert,
      severity: drift.alert ? (drift.overallDrift > threshold * 2 ? "critical" : "warning") : "info",
      title: "Allocation Drift",
      message: drift.alert
        ? `Max drift at ${drift.overallDrift}% (threshold: ${threshold}%).`
        : `Drift within bounds at ${drift.overallDrift}%.`,
      data: { categories: drift.categories, maxDrift: drift.overallDrift, threshold },
    });
  }
