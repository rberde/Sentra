...
import { calculateDriftCategories, getOverallDrift } from "@/lib/engine/monitoring";
...
export async function GET() {
  ...
  const categories = calculateDriftCategories([
    { name: "Fixed Expenses", actual: actualFixed, planned: reallocation?.fixedExpenses },
    { name: "Variable Expenses", actual: actualVariable, planned: reallocation?.variableExpenses },
    { name: "Investments", actual: actualInvestment, planned: reallocation?.investments },
  ]);
  ...
  const overallDrift = getOverallDrift(categories);
