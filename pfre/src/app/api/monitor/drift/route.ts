...
import { calculateDollarDrift } from "@/lib/monitoring";
...
export async function GET() {
  ...
  const categories = [
    calculateDollarDrift("Fixed Expenses", actualFixed, reallocation?.fixedExpenses ?? 0),
    calculateDollarDrift("Variable Expenses", actualVariable, reallocation?.variableExpenses ?? 0),
    calculateDollarDrift("Investments", actualInvestment, reallocation?.investments ?? 0),
  ];
