...
import { getAllocationDriftCategories } from "@/lib/engine/monitoring";
...
export async function GET() {
  ...
  const driftCategories = getAllocationDriftCategories({
    income,
    actualFixed,
    actualVariable,
    actualInvestment,
  ...
  const categories = driftCategories.map(category => ({
    name: category.name,
