...
export async function GET() {
  ...
  const spending = calculateSpendingUsage(actualSpending, reallocation, spendingRule?.threshold as number | undefined);
  ...
  return NextResponse.json({
    ...
    data: {
      planActive: true,
      variableSpendingCap: spending.variableCap,
      actualSpendingThisMonth: actualSpending,
      percentUsed: spending.percentUsed,
      thresholdPercent: spending.thresholdPercent,
      daysRemainingInMonth: daysRemaining,
      alert: spending.alert,
      alertLevel: spending.alertLevel,
      message: spending.alert
        ? `Variable spending at ${spending.percentUsed}% of plan cap ($${actualSpending} / $${spending.variableCap}).`
