...
export async function POST(req: Request) {
  try {
    ...
    try {
      const transactions = await getTransactionsWithRetry(accessToken, startDate, endDate);
      ...
      // Transactions may be unavailable for some institutions/items; continue with balances-only autofill.
    ...
    try {
      ...
      // Investments product may not be available for this item
    }
    ...
    try {
      const liabResponse = await plaidClient.liabilitiesGet({
        access_token: accessToken,
      });
      const creditLiabs = (liabResponse.data.liabilities.credit ?? []).map(c => ({
        type: "credit" as const,
        name: `Credit Card ...${(c.account_id ?? "").slice(-4)}`,
        balance: c.last_statement_balance ?? 0,
        apr: c.aprs?.[0]?.apr_percentage ?? null,
        minimumPayment: c.minimum_payment_amount ?? null,
      }));
      const studentLiabs = (liabResponse.data.liabilities.student ?? []).map(s => ({
        type: "student_loan" as const,
        name: s.loan_name ?? "Student Loan",
        balance: s.outstanding_interest_amount
          ? (s.origination_principal_amount ?? 0) + s.outstanding_interest_amount
          : s.origination_principal_amount ?? 0,
        apr: s.interest_rate_percentage ?? null,
        minimumPayment: s.minimum_payment_amount ?? null,
      }));
      liabilities = [...creditLiabs, ...studentLiabs];
    } catch {
    ...
    return NextResponse.json({
      ...
      autofillMeta: {
        ...
        liabilitiesAutofilled: liabilities.length > 0,
        expensesReason,
      },
    });
  } catch (error: unknown) {
...
function buildExpenseEstimates(transactions: Array<{
  ...
  for (const entry of grouped.values()) {
    ...
    const looksFixed = entry.occurrences >= 2 || entry.category === "housing" || entry.category === "insurance" || entry.category === "loans" || entry.category === "subscriptions" || looksRecurringByName;
    if (looksFixed) {
